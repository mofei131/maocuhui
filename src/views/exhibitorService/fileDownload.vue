<template>
    <div class="right_Content">
     <!-- <div class="title">
        <span class="title-text">{{ $t('exhibitorService.t4') }}</span>
        <div class="mfsek"></div>
      </div> -->
      <div class="titles"><span class="title-texts">{{ $t('exhibitorService.t4') }}</span></div>
    ...
    getDataList() {
      this.$http({
        url: this.$http.adornUrl(
          `/file/list?asc=seq&page=${this.pageIndex}&pageSize=${this.pageSize}&type=1`
        ),
        method: "get",
      }).then(({ data }) => {
        if (data && data.code === 1) {
