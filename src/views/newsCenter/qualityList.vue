  ...
  <div>
    <div class="right_Content">
      ...
      <div class="font_content">
        <div class="mftit">新产品新项目</div>
        <ul class="mediaCoverageUl">
          <li v-for="(item, index) in dataList1" :key="index">
            <div class="dociv">
              <span class="name">{{ item.title }}</span>
              <img class="contentImg" :src="item.picture" />
<!--              <router-link :to="'/newsCenter/qualityInfo?id=' + item.id" class="past">

              </router-link>-->
            </div>
          </li>
        </ul>
        <div class="mftit">新服务新业态</div>
        <ul class="mediaCoverageUl">
...
export default {
  ...
  methods: {
    getDataList1() {
      this.$http({
        url: this.$http.adornUrl(`/quality/list`),
        method: 'get',
